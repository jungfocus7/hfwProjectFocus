...
<div style="background-color: #748B96;
    ...
    <span style="position: relative;
        ...
        font-size: 11px; color:rgba(255, 255, 255, 0.32);"></span>
</div>
    `.trim();
};
...
class HcScrollBar extends EventTarget {
    ...
    constructor(targetArea, type, eid) {
        ...
        if (gdo.logicType === HeScrollType.both) {
            ...
            throw new Error('logicType is not set');
            // return;
        }
    ...
    #fn_printSpanLog() {
        ...
        if (gdo.logicType === HeScrollType.both) {
            const tshg = (100 * gdo.hgr).toFixed(1);
            const tshsp = (100 * gdo.hspr).toFixed(1);
            const tsvg = (100 * gdo.vgr).toFixed(1);
            const tsvsp = (100 * gdo.vspr).toFixed(1);
            const txt = `
${tshg}% / ${tshsp}%
${tsvg}% / ${tsvsp}%
            `.trim();
            ...
            const tshg = (100 * gdo.hgr).toFixed(1);
            const tshsp = (100 * gdo.hspr).toFixed(1);
            const txt = `${tshg}% / ${tshsp}%`;
            gdo.heSpan.innerText = txt;
        }
        else if (gdo.logicType === HeScrollType.vertical) {
            const tsvg = (100 * gdo.vgr).toFixed(1);
            const tsvsp = (100 * gdo.vspr).toFixed(1);
            const txt = `${tsvg}% / ${tsvsp}%`;
            gdo.heSpan.innerText = txt;
        }
    }
     ...
     * @returns  {void}
     */
    setHoriGroundRatio(tv) {
        const gdo = this.#gdo;

        if (tv === gdo.hgr) return;
        gdo.hgr = HsDoubleHelper.checkRatio(tv);

        this.#fn_calcUpdateThumbWidth();
        this.#fn_calcUpdateThumbLeft();

        this.#fn_applyRectThumb(false);
        this.#fn_printSpanLog();
    }
     ...
     * @returns  {void}
     */
    setVertGroundRatio(tv) {
        const gdo = this.#gdo;

        if (tv === gdo.vgr) return;
        gdo.vgr = HsDoubleHelper.checkRatio(tv);
     ...
     * @returns {void}
     */
    setHoriScrollRatio(tv) {
        const gdo = this.#gdo;

        if (tv === gdo.hspr) return;
        gdo.hspr = HsDoubleHelper.checkRatio(tv);
     ...
     * @returns {void}
     */
    setVertScrollRatio(tv) {
        const gdo = this.#gdo;

        if (tv === gdo.vspr) return;
        gdo.vspr = HsDoubleHelper.checkRatio(tv);
